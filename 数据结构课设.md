...
void Creat(Mgraph *g) {//  **构建无向图**
	...
	if (vs && rf) {
		...
		printf("error: can't open this file, and n==0;\n");
		exit(0);
	}
}
int visited[M];
...
void printout(Path p) {
	while (!isEmpty(p)) {
		printf("%d->", p.path[p.top]);
		p.top--;
	}
	printf("\n");
}
...
void PrintAllPath(Mgraph *g) {
	...
	Gnode->top = 0;
	push(*Gnode, g->vexs[start]);
	isvisited[start] = true;
	int top_node;
	int pop_node = -1;
	while (!isEmpty(*Gnode)) {
		top_node = Gnode->path[Gnode->top];
		int k;
